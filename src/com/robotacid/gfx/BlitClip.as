package com.robotacid.gfx {
	...
	*/
	public class BlitClip extends BlitSprite{
		...
		public function BlitClip(displayObject:DisplayObject = null, colorTransform:ColorTransform = null, fadeFrames:int = 0) {
			frames = new Vector.<BitmapData>();
			if(displayObject){
				super(displayObject, colorTransform);
				frames[0] = data;
				var i:int;
				if(fadeFrames){
					if(!colorTransform) colorTransform = new ColorTransform();
					var alphaStep:Number = colorTransform.alphaMultiplier / fadeFrames;
					colorTransform.alphaMultiplier -= alphaStep;
					for(i = 1; i < fadeFrames; i++){
						totalFrames = fadeFrames;
						frames[i] = new BitmapData(Math.ceil(bounds.width), Math.ceil(bounds.height), true, 0x0);
						frames[i].draw(displayObject, new Matrix(1, 0, 0, 1, -bounds.left, -bounds.top), colorTransform);
						colorTransform.alphaMultiplier -= alphaStep;
					}
				} else {
					var mc:MovieClip = displayObject as MovieClip;
					for(i = 2; i < mc.totalFrames + 1; i++){
						mc.gotoAndStop(i);
